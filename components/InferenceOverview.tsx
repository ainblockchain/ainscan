...
export default async function InferenceOverview({ publisher }: { publisher?: string }) {
  if (!publisher || !/^0x[a-fA-F0-9]{40}$/.test(publisher)) return null;
  let overview: ReturnType<typeof inferenceOverview> | null = null;
  try {
    overview = inferenceOverview(publisher, await getValue(`${INFERENCE_ROOT}/${publisher}`));
  } catch {}
